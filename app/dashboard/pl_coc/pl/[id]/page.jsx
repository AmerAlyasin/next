...
const SinglePl = ({params}) => {
  ...
  useEffect(() => {
    const getPlById = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/pl/${params.id}`,{
          method: "GET",
        });
        if (!response.ok) {
    ...
    getPlById();
      ...
      const downloadPlPdfDocument = async () => {
        try {
          ...
          // Send data to the server to create the document
          const response = await fetch('http://localhost:3000/api/loadPlPdf', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  return ( 
    ...
    <div> 
      <form onSubmit={handleSubmit}>
        ...
        <button type="button" className={styles.DownloadButton} onClick={downloadPlPdfDocument}>
           Download Picklist
           </button>
